...
def predict():
    ...
    try:
        if request.method == 'POST':
            ...
            corrected_symptoms = [correct_spelling(symptom) for symptom in symptoms]
            corrected_symptoms = list(filter(None, corrected_symptoms))

            if len(corrected_symptoms) == 0:
                error = "Please enter at least one valid symptom."
                return render_template('index.html', error=error)

            predicted_disease = predicted_value(corrected_symptoms)
            dis_des, my_precautions, medications, my_diet, workout = information(predicted_disease)
            
            return render_template('index.html', predicted_disease=predicted_disease,
                                   dis_des=dis_des, my_precautions=my_precautions,
                                   medications=medications, my_diet=my_diet, workout=workout, symptoms=symptoms_input)
