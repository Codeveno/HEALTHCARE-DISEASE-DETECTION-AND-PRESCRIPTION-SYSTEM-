...
def predicted_value(patient_symptoms):
    ...
    for i in patient_symptoms:
        i_vector[symptoms_list_processed.get(i)] = 1
    prediction = Rf.predict([i_vector])
    return diseases_list[int(prediction)]
...
# Users dictionary and registration
users = {
...
def predict():
    ...
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            ...
            disease_desciption, disease_precautions, disease_medications, disease_diet, disease_workout = information(predicted_disease)

            return render_template('index.html', predicted_disease=predicted_disease,
                                   disease_desciption=disease_desciption, disease_precautions=disease_precautions,
                                   disease_medications=disease_medications, disease_diet=disease_diet,
                                   disease_workout=disease_workout)
